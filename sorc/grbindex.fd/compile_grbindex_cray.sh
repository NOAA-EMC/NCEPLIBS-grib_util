...
######################################################################
#
# This script uses to build grbindex on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The grbindex only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for grbindex source code directory
#
cd  $dir
export chkfile=$dir/grbindex.f

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### grbindex source code NOT found  #####"
    echo " "
    echo " Building executable grbindex can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-grbindex_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-grbindex_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-grbindex_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-grbindex_intel_sandybridge.log
   mv grbindex ../../exec &>>compile-grbindex_intel_sandybridge.log
   echo " "
   echo " "
   echo " "

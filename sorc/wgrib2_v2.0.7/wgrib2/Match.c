...
char *match_extra_fn_arg3[MATCH_EXTRA_FN] = { NULL } ;
int match_extra_fn_n;

/*
 * HEADER:100:match_inv_add:setup:1:add new options to match_inventory
 */
int f_match_inv_add(ARG1) {
    int j;
	...
	if (j == nfunctions) fatal_error("match_inv_add: %s not found", arg1);
	    ...
	    match_extra_fn_nargs[match_extra_fn_n] = 0;
            match_extra_fn_n++;
        }

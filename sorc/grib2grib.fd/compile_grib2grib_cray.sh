...
######################################################################
#
# This script uses to build grib2grib on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The grib2grib only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for grib2grib source code directory
#
cd  $dir
export chkfile=$dir/grib2grib.f

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### grib2grib source code NOT found  #####"
    echo " "
    echo " Building executable grib2grib can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-grib2grib_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-grib2grib_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-grib2grib_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-grib2grib_intel_sandybridge.log
   mv grib2grib ../../exec &>>compile-grib2grib_intel_sandybridge.log
   echo " "
   echo " "
   echo " "

...
!  DO SUBSECTION OF GRID IF KGDSO(1) IS SUBTRACTED FROM 255.
 ...
 ELSE
   KGDSO(1)=255+KGDSO(1)
   CALL GDSWZD(KGDSO,-1,MO,FILL,XPTS,YPTS,RLON,RLAT,NO)
   IF(NO.EQ.0) IRET=3
 ENDIF
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  SET PARAMETERS
     ...
     DO K=1,KM
       DO N=1,NO
         IF(N11(N).GT.0) THEN
           IF(IBI(K).EQ.0) THEN
!cdz+5
             IF(GI(N11(N),K) == FILL9 .OR. GI(N21(N),K) == FILL9 .OR. &
                GI(N12(N),K) == FILL9 .OR. GI(N22(N),K) == FILL9) THEN
                GO(N,K)=FILL9
                LO(N,K)=.FALSE.
             ELSE

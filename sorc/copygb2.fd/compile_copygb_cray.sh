...
######################################################################
#
# This script uses to build copygb on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The copygb only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for copygb source code directory
#
cd  $dir
export chkfile=$dir/copygb.F

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### copygb source code NOT found  #####"
    echo " "
    echo " Building executable copygb can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-copygb_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-copygb_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-copygb_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-copygb_intel_sandybridge.log
   mv copygb ../../exec &>>compile-copygb_intel_sandybridge.log
   echo " "
   echo " "
   echo " "

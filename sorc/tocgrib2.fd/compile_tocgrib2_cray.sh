...
######################################################################
#
# This script uses to build tocgrib2 on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The tocgrib2 only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for tocgrib2 source code directory
#
cd  $dir
export chkfile=$dir/tocgrib2.f

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### tocgrib2 source code NOT found  #####"
    echo " "
    echo " Building executable tocgrib2 can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-tocgrib2_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-tocgrib2_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-tocgrib2_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-tocgrib2_intel_sandybridge.log
   mv tocgrib2 ../../exec &>>compile-tocgrib2_intel_sandybridge.log
   echo " "
   echo " "
   echo " "

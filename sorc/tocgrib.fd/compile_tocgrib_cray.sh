...
######################################################################
#
# This script uses to build tocgrib on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The tocgrib only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for tocgrib source code directory
#
cd  $dir
export chkfile=$dir/tocgrib.f

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### tocgrib source code NOT found  #####"
    echo " "
    echo " Building executable tocgrib can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-tocgrib_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-tocgrib_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-tocgrib_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-tocgrib_intel_sandybridge.log
   mv tocgrib ../../exec &>>compile-tocgrib_intel_sandybridge.log
   echo " "
   echo " "
   echo " "

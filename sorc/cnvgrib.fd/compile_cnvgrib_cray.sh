...
######################################################################
#
# This script uses to build cnvgrib on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The cnvgrib only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for cnvgrib source code directory
#
cd  $dir
export chkfile=$dir/cnvgrib.f

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### cnvgrib source code NOT found  #####"
    echo " "
    echo " Building executable cnvgrib can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-cnvgrib_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-cnvgrib_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-cnvgrib_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-cnvgrib_intel_sandybridge.log
   mv cnvgrib ../../exec &>>compile-cnvgrib_intel_sandybridge.log
   echo " "
   echo " "
   echo " "

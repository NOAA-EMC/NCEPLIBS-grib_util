...
######################################################################
#
# This script uses to build degrib2 on CRAY (craype-sandybridge)
...
if  [ $machine_type = l -o $machine_type = s ] ; then # For CRAY
   ...
   echo "  "
   echo "  The degrib2 only builds on Luna/Surge machine. "
   echo "  "
   echo "  "
   echo "  "
   exit
...
#
# Checking for degrib2 source code directory
#
cd  $dir
export chkfile=$dir/degrib2.f

if  [  ! -f $chkfile ] ; then
    echo " "
    echo " "
    echo " ##### degrib2 source code NOT found  #####"
    echo " "
    echo " Building executable degrib2 can not continue.  Aborted ! "
    echo " "
    echo " "
...
# Loading IOBUF
   ...
   module list 2>compile-degrib2_intel_sandybridge.log
   module list
   echo " "
   echo " "
   ...
   make -f makefile_cray &>>compile-degrib2_intel_sandybridge.log
   make -f makefile_cray clean &>>compile-degrib2_intel_sandybridge.log
   mkdir -p ../../exec &>>compile-degrib2_intel_sandybridge.log
   mv degrib2 ../../exec &>>compile-degrib2_intel_sandybridge.log
   echo " "
   echo " "
   echo " "
